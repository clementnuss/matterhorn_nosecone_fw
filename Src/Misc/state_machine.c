	...
	case STATE_COAST:
	  {
	    ...
	    if (baroIsReady)
	      {
		uint8_t minAltTrig = ((baro_data->altitude
		    - calib_initial_altitude) > ROCKET_CST_MIN_TRIG_AGL);
	...
	case STATE_PRIMARY:
	  {
		...
		uint8_t sensorMuteTimeTrig = ((HAL_GetTick () - time_tmp)
		    > APOGEE_MUTE_TIME);
		uint8_t counterSecTrig = 0;

		// update the minimum altitude detected up to this point
		if ((baro_data->altitude - calib_initial_altitude)
		    > ROCKET_CST_REC_SECONDARY_ALT)
		  {
		...
		if (baroIsReady)
		  {
		    ...
		    if (abs_fl32 (
			baro_data->altitude - td_last_alt) > TOUCHDOWN_ALT_DIFF)
		      {
