...
#define MAX_OPENING_DEG 190//To be confirmed..  IN WINGS REFERENTIAL
...
{
  int inc = -(int) (degrees_angle * 189); //3000 inc/evolution, 1:25reductor
  return inc;
}
...
void motor_goto_position_inc (int position_inc)
{
  ...
  command_string[strlen (command_string)] = 'M';
  command_string[strlen (command_string)] = '\r';

  HAL_UART_Transmit (airbrake_huart, (char*) command_string, strlen (command_string), 30);
  return;
}
...
//	HAL_UART_Transmit(airbrake_huart, command, 8, 30);
  ...
  command = "PD1\r";
  HAL_UART_Transmit (airbrake_huart, command, 4, 30);
  command = "LPC2000\r"; // peak current max, to be redefined
  HAL_UART_Transmit (airbrake_huart, command, 8, 30);
  command = "LCC1500\r"; // continuous current max_to be redefined
  HAL_UART_Transmit (airbrake_huart, command, 8, 30);
  //Enable
  command = "EN\r";
  HAL_UART_Transmit (airbrake_huart, command, 3, 30);
...
//    float opt_act_position_deg = 0.0;
  ...
  //On passe en mode controle PID que lorsque l'on est � l'int�rieur de la bande de controle
  if (opt_act_position_inc <= full_close_inc || opt_act_position_inc >= full_open_inc)
  ...
  else
    {
      ...
      command_inc = opt_act_position_inc - Kp_correction_margin * inc_error
          - Td_correction_margin * (inc_error - correction_margin_last_error)
          - Ki_correction_margin * correction_margin_error_integration;
